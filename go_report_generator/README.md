...
*   **Entrypoint (`cmd/main.go`):** The main application entry point handles command-line argument parsing and orchestrates the report generation workflow.
...
*   **Parser Factory (`internal/parser`):** At the core of the input processing is a parser factory. It dynamically detects the coverage report format (e.g., Cobertura XML, Go cover profile) and returns the appropriate parser implementation (`IParser`). This makes it easy to add support for new formats by simply creating a new parser and registering it with the factory.
*   **Language-Specific Processing (`internal/language`):** To handle language-specific nuances like formatting C#/.NET identifiers or calculating Go-specific metrics, a language processor factory (`language.Processor`) is used. This allows for clean separation of concerns, where parsers focus on format and processors focus on language semantics.
